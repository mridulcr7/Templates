...
ll get(vector<ll> &t, ll v, ll tl, ll tr, ll pos)
{
    if (tl == tr)
        return t[v];
    ll tm = (tl + tr) / 2;
...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        bb[i].push_back(summ(x));
    }
    vector<ll> aa(n, 0);
    vector<ll> b(4 * n, 0);
    build(b, aa, 1ll, 0, n - 1);
    for (int i = 0; i < q; i++)
    {
        cin >> x;
        if (x == 1)
        {
            ll l, h;
            cin >> l >> h;
            update(b, 1ll, 0, n - 1, l - 1, h - 1, 1ll);
        }
        else
        {
            cin >> y;
            ll idx = 0;
            idx = get(b, 1ll, 0, n - 1, y - 1);
            // cout << idx << " ";
            if (bb[y - 1].size() <= idx)
            {
                cout << bb[y - 1][bb[y - 1].size() - 1] << "\n";
            }
            else
            {
                cout << bb[y - 1][idx] << "\n";
