...
ll xor_query(vector<ll> &b, ll curr, ll l, ll h, ll low, ll high)
{
    if (l > h)
    {
        return 0ll;
    ...
    ll mid = (low + high) / 2;
    return xor_query(b, curr * 2, l, min(h, mid), low, mid) ^ xor_query(b, curr * 2 + 1, max(mid + 1, l), h, mid + 1, high);
}
...
void solve()
{
    ...
    for (int i = 0; i < q; i++)
    {
        ...
        {
            cout << xor_query(b, 1ll, l - 1, h - 1, 0ll, n - 1) << "\n";
        }
    }
}
